...
초기 목표였던 ‘팀원 모두가 데이터 전처리부터 모델링까지 전 과정 경험’은 성공적으로 달성되었다. 개인별 아이디어를 바탕으로 전 과정을 수행하며 역량을 내재화했고, 팀으로서 서울시 아파트 실거래가 예측 경진대회에서 5위(RMSE 46950.6270)라는 우수한 성과를 거두었다. 결과적으로 개인 성장과 팀 경쟁력 두 마리 토끼를 잡는 의미 있는 프로젝트였다.
